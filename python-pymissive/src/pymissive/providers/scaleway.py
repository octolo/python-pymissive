...
class ScalewayProvider(MissiveProviderBase):
    ...
    def get_normalize_recipients(self, response: dict[str, Any]) -> list:
    ...
    def _prepare_email(self, **kwargs):
        ...
        self._add_sender(kwargs["sender_name"], kwargs["sender_email"])
    ...
    def _add_sender(self, sender_name, sender_email):
        prefix, suffix = sender_email.split("@")
    ...
    def status_email(self, **kwargs) -> list[dict[str, Any]]:
        ...
        for recipient in kwargs.get("recipients", []):
            ...
            data = resp.json()
            evt_type = data.get("type") or data.get("status", "unknown")
            events.append({
