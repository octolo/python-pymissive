...
def test_send_lre_success_is_request_for_any_2xx(status_code):
    provider = MailevaProvider.__new__(MailevaProvider)
    provider._stage_lre_before_submit = lambda **kwargs: ("ext-1", [], [])
    provider.get_endpoint = lambda name: "%s/submit"
